    ...
    def classificar_dados(self):
        ...
        if os.path.exists('contas.csv'):
            with open('contas.csv', mode='r', newline='') as file:
                ...
                for row in reader:
                    if row and row[0] == self.empresa_entry.get() and row[1] == self.banco_entry.get():
                        conta_ativo = row[4]  #? conta ativo está na coluna 5
                        break

        print(f"Conta ativo encontrada: {conta_ativo}")  #? debug
        ...
        for item in self.tree.get_children():
            ...
            if not correspondencia.empty:
                ...
                debito = str(correspondencia.iloc[0, 6]).strip()  # coluna G (debito) no banco
                credito = str(correspondencia.iloc[0, 9]).strip()  # coluna J (credito) no banco

                print(f"Valores encontrados - Débito: '{debito}', Crédito: '{credito}'")  # Debug

                #? substituir #BCO pela conta ativo se existir
                if conta_ativo:
                    if debito == "#BCO":
                        print(f"Substituindo débito #BCO por {conta_ativo}")  # Debug
                        debito = conta_ativo
                    if credito == "#BCO":
                        print(f"Substituindo crédito #BCO por {conta_ativo}")  # Debug
                        credito = conta_ativo
