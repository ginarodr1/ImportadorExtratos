...
class ImportadorExtratos:
    ...
    def classificar_dados(self):
        ...
        for item in self.tree.get_children():
            ...
            lancamento_idx = self.tree["columns"].index("LançamentoLC")
            values[lancamento_idx] = lancamento_simplificado
            
            # Adicionar identificação do banco se solicitado
            if incluir_banco:
        ...
        # Classificar débito e crédito
        for item in self.tree.get_children():
            values = list(self.tree.item(item, 'values'))
            descricao = values[1]  # Descrição está na segunda coluna
            ...
            if not correspondencia.empty:
                debito = str(correspondencia.iloc[0]['Debito']).strip()
                credito = str(correspondencia.iloc[0]['Credito']).strip()
                ...
                values[9] = credito  # Coluna de crédito
                self.tree.item(item, values=values)
